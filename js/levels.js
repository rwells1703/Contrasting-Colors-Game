	...
	while (i < level.height) {
		...
		while (j < level.width) {
			...
			if (stringified== JSON.stringify(itemTypes.platform)) {
                scene.platformsArr.push(new Platform(scene.platforms, COLORS.white, positionToPx(j), positionToPx(i)));
            } else if (stringified == JSON.stringify(itemTypes.player)) {	
                scene.player = new Player(scene, COLORS.black, 10, positionToPx(j), positionToPx(i));
